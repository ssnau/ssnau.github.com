    ...
    return {
        link: function(scope, element, attr) {
            ...
            editor.getSession().on('change', function(){
                ...
                timer = setTimeout(scope.validate, 200);
            });

            element.click(scope.validate);
            setTimeout(scope.validate, 1);//make the function call out of the context, so that scope.apply can be ok.
        }
    }
})
     ...
     $scope.validate = function(){
            ...
            if (!errors.length) { //只有当错误数量为0时，才执行scope检查
                scopeManager = escope.analyze(res, {
                    globals: [].concat(browser, node, rhino, standard, windows, mstr, user_global)
                });
